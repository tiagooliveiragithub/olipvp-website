...
class Subscription(models.Model):
    ...
    image = models.ImageField(upload_to='subscriptions/', null=False, blank=False)
    color = models.CharField(max_length=120, help_text="Color for membership", blank=True, null=True)
...
class SubscriptionPrice(models.Model):
    ...
    def display_image(self):
        if not self.subscription:
            return None
        return self.subscription.image.url

    @property
    def get_color_subscription(self):
        if not self.subscription:
            return None
        return self.subscription.color
